...
如 Frege 氏在后记所言，他认为悖论的产生要归咎于基本定律 V。但是 Hampkins, J. *认为 Frege 氏这一归咎是错误的。在当代解读中，基本定律 V 等同于如下的类外延原理
...
**「普遍概括原理」** 对于任何性质 $\varphi$，可以形成一个所有具有性质 $\varphi(x)$ 的 $x$ 的集合，即 $$\\{x|\varphi(x)\\}$$ 是一个集合。
...
这一悖论相当于说，我们不能拥有一个对象的总体，使得每一个总体中对象的类 $\\{x|Fx\\}$ 都对应于一个总体中的外延对象 $\varepsilon F$。Hampkin 氏的结论是，要归咎的不是基本定律 V，而是「普遍概括原理」。
