...
Leslie, A. 发育模型假设具有心智模块理论的完整的元表征的出现早于发育过程，且跟佯装有关。比如13月幼崽会佯装香蕉是电话。Leslie 氏声称这是一个关于表征的想法。在他的「佯装解耦模型」，<香蕉>这类原初表征从通常功能中解耦来表示其他事情。该氏于是认为幼崽必须察觉到<香蕉>这个概念，才可以把这个引用修改着玩。
