...
我假定读者均受过初等和中等逻辑训练，也就是说明白逻辑的形式化以及命题逻辑、谓词逻辑、一阶逻辑的形式语言和证明系统。换句话说，给不需要接触高等逻辑的非相关专业学生一个高等逻辑在干什么的一瞥。
...
在逻辑学家看来，一个逻辑（a logic）由所有真命题，或者所有正确推理构成。要描述正确推理，我们可以借助公理和推理规则两种方式。所有的证明系统都由这两种方式组合而成，证明系统也是定义一个形式系统必需的数据。
...
这里 $Provable(-)$ 是个形式系统里的性质，按照形式系统的功能捕捉了某个数论命题 P(-)，但是这个数论命题经过精心构造反映了「命题在 PA 可证」这个元性质。
...
根据 Godel 第二不完备性定理，PA 系统不能证明自身的一致性。但是 Gentzen 证明了如果将证明系统里的归纳法换成（无量词的）上至 $\varepsilon_0$ 的超限归纳法，那 PA 的一致性仍然可以在内部证明。

---

Godel 的这个构造工作，对理论家更重要的启发是在证明系统中，除了「可证」这个概念以外，还需要关注「可枚举」和「可判定」，比如之前介绍的「可有效枚举」。换句话说，在仅有算术资源可以利用的证明系统中，这个证明系统产生证明的能力，也就是计算能力，是遭到限制的。若干年后 Turing, A. 发表了一套关于计算的数学理论，就是为了刻画 Godel 的理论中的这个计算能力的限制到底是什么。